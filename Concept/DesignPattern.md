...
3. **里氏替換原則（Liskov Substitution Principle，LSP）**：
   所有引用基底類別的地方必須能夠透明地使用其子類別的物件，即子類別可以替換父類別並且不會影響程式的正確性。這意味著子類別應該符合父類別所製定的契約或規範。

4. **介面隔離原則（Interface Segregation Principle，ISP）**：
   客戶端不應該被迫依賴它不使用的介面，即應該將大介面拆分成更小的、更具體的介面，以便客戶端只需要知道它們感興趣的介面。

5. **依賴倒置原則（Dependency Inversion Principle，DIP）**：
   高層模組不應該依賴低層模組，二者都應該依賴抽象；抽象不應該依賴具體實現，具體實現應該依賴抽象。這意味著應該透過介面和抽象來解耦模組之間的依賴關係。

6. **合成/聚合複用原則（Composite/Aggregate Reuse Principle，CARP）**：
